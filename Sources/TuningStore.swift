...
final class TuningStore: ObservableObject {
    ...
    func applyReadValues(_ values: [Int: Double]) {
        ...
        didLoadFromController = true
        isReading = false
        statusText = "Loaded current settings"
        saveBackup(reason: "auto-read")
    }

    func applyFallbackLoadedValuesIfNeeded() {
        guard !didLoadFromController else { return }
        var values: [Int: Double] = [:]
        for p in parameters {
            values[p.id] = p.currentValue ?? p.originalValue ?? 0
        }
