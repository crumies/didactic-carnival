                ...
                GlassCard(glow: tuning.didLoadFromController) {
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        HStack {
                            ...
                            .tint(.cyan)
                            .disabled(!ble.isConnected || tuning.isReading)
                        ...
                        if let url = tuning.lastBackupURL {
                    ...
                    ForEach(filtered) { param in
                        ParameterRow(param: param, disabled: !tuning.didLoadFromController) { newValue in
                            if param.kind == .toggle {
                                ...
                                tuning.updatePending(id: param.id, value: newValue)
                            }
                        }
                    }
    ...
    private var lockedCard: some View {
        GlassCard(glow: true) {
            VStack(spacing: 12) {
                ...
                Text("Press unlock to access parameter controls.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
    ...
    var body: some View {
        GlassCard(glow: param.hasChange) {
            VStack(alignment: .leading, spacing: 12) {
                ...
                if param.kind == .toggle {
                    ...
                    VStack(spacing: 8) {
                        HStack {
                            Text("Value")
                            Spacer()
                            Text(String(format: "%.2f", param.pendingValue ?? param.currentValue ?? 0))
                                .fontWeight(.semibold)
                        }

                        Slider(
                            value: Binding(
                                get: { param.pendingValue ?? param.currentValue ?? param.min },
                                set: { onChange($0) }
                            ),
                ...
                if param.hasChange {
                    Text("Changed: original \(String(format: "%.2f", param.currentValue ?? 0)) → new \(String(format: "%.2f", param.pendingValue ?? 0))")
                        .font(.caption2)
