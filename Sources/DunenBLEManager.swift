...
final class DunenBLEManager: NSObject, ObservableObject {
    ...
    func readCurrentSettings() {
        ...
        let frame = DunenProtocol.readAllParametersFrame()
        p.writeValue(frame, for: c, type: .withResponse)

        // Never default toggles to 0 on connect. If no response arrives, keep controls locked.
    ...
    func writeChangedSettings(_ params: [TuningParameter]) {
        ...
        for param in params {
            ...
            p.writeValue(frame, for: c, type: .withResponse)
            writtenIDs.append(param.id)
        }

        DispatchQueue.main.asyncAfter(deadline: .now() + 1.5) {
            self.tuningStore?.confirmWritten(ids: writtenIDs)
        }
    }
    ...
    private func addPacket(_ data: Data) {
        ...
        if !parsed.isEmpty {
            tuningStore?.applyReadValues(parsed)
        }
    }
    ...
    private func decodeTelemetry(_ data: Data) {
        ...
        // Best-effort telemetry only. Raw packet logger stays visible for exact mapping.
        if b.count >= 12 {
        ...
        if b.count > 15 {
            ...
            telemetry.dcCurrent = Double(Int8(bitPattern: b[10]))
            telemetry.phaseCurrent = Double(UInt16(b[11]) | (UInt16(b[12]) << 8)) / 10.0
            telemetry.motorTemp = Double(Int8(bitPattern: b[13]))
            telemetry.controllerTemp = Double(Int8(bitPattern: b[14]))
            telemetry.torque = Double(Int8(bitPattern: b[15]))
        }
    }
...
extension DunenBLEManager: CBPeripheralDelegate {
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverCharacteristicsFor service: CBService, error: Error?) {
        ...
        connectionStatus = "FFE0 ready. Press Read Current Settings."
    }
    ...
    func peripheral(_ peripheral: CBPeripheral, didUpdateNotificationStateFor characteristic: CBCharacteristic, error: Error?) {
        ...
        if characteristic.isNotifying {
            connectionStatus = "Receiving live packets"
        }
    }
}
